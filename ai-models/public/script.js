...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const textToImageDiv = document.querySelector(".text-to-image");
    ...
    if (textToImageDiv) {
        const submitButton = textToImageDiv.querySelector(
          '.left-editor form input[type="submit"]'
        );
        const textarea = textToImageDiv.querySelector(
			".left-editor form textarea"
		);
		...
		const validateInput = () => {
			const textLength = textarea.value.trim().length;
			submitButton.disabled = !(textLength > 15 && textLength < 100000);
		};
